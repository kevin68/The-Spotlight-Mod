...
public class GuiSpotLight extends GuiContainer {
	...
	public void initGui() {
		...
		this.buttonColors = this.addButton(new GuiButton(0, x + 5, y + 20, 80, 20, this.tile.isBeam ? I18n.format("container.spotlight.color") : I18n.format("container.spotlight.textcolor")) {
			...
			public void onClick(double mouseX, double mouseY) {
				if (GuiSpotLight.this.buttonMode.isActive()) {
					TSMNetwork.CHANNEL.sendToServer(new PacketOpenGui(GuiSpotLight.this.tile.getPos().getX(), GuiSpotLight.this.tile.getPos().getY(), GuiSpotLight.this.tile.getPos().getZ(), 4));
				} else {
		...
		this.buttonTextures = this.addButton(new GuiButton(3, x + 5, y + 89, 80, 20, I18n.format("container.spotlight.textures")) {
			@Override
			public void onClick(double mouseX, double mouseY) {
				TSMNetwork.CHANNEL.sendToServer(new PacketOpenGui(GuiSpotLight.this.tile.getPos().getX(), GuiSpotLight.this.tile.getPos().getY(), GuiSpotLight.this.tile.getPos().getZ(), 1));
			}
		});
		...
		this.addButton(new GuiButton(5, x + 90, y + 20, 80, 20, I18n.format("container.spotlight.timeline")) {
			@Override
			public void onClick(double mouseX, double mouseY) {
				TSMNetwork.CHANNEL.sendToServer(new PacketOpenGui(GuiSpotLight.this.tile.getPos().getX(), GuiSpotLight.this.tile.getPos().getY(), GuiSpotLight.this.tile.getPos().getZ(), 3));
			}
		});
		this.addButton(this.buttonLock = new GuiBooleanButton(17, x + 180, y + 65, 20, 20, "", this.tile.locked) {
			@Override
			public void onClick(double mouseX, double mouseY) {
				TSMNetwork.CHANNEL.sendToServer(new PacketLock(GuiSpotLight.this.tile.getPos().getX(), GuiSpotLight.this.tile.getPos().getY(), GuiSpotLight.this.tile.getPos().getZ(),
						GuiSpotLight.this.buttonLock.isActive(), GuiSpotLight.this.mc.player.getGameProfile().getId()));
			}
		});
		...
		this.addButton(new GuiButton(19, x + 180, y + 115, 20, 20, "") {
			@Override
			public void onClick(double mouseX, double mouseY) {
				TSMNetwork.CHANNEL.sendToServer(new PacketOpenGui(GuiSpotLight.this.tile.getPos().getX(), GuiSpotLight.this.tile.getPos().getY(), GuiSpotLight.this.tile.getPos().getZ(), 2));
			}
		});
		this.addButton(this.buttonHelp = new GuiBooleanButton(20, x + 180, y + 140, 20, 20, "?", this.tile.helpMode) {
			@Override
			public void onClick(double mouseX, double mouseY) {
				GuiSpotLight.this.buttonHelp.toggle();
				GuiSpotLight.this.tile.helpMode = GuiSpotLight.this.buttonHelp.isActive();
			}
		});
	...
	public void onGuiClosed() {
		TSMNetwork.CHANNEL.sendToServer(
				new PacketUpdateData(this.tile.getPos().getX(), this.tile.getPos().getY(), this.tile.getPos().getZ(), this.tile.dimension, TSMJsonManager.getDataFromTile(this.tile).toString()));
		super.onGuiClosed();
	}
