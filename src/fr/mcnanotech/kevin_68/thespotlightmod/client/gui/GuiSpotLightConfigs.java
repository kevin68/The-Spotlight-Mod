...
public class GuiSpotLightConfigs extends GuiContainer
{
	...
	public void initGui()
	{
		...
		buttonList.add(new GuiButton(0, x + 38, y + 112, 100, 20, I18n.format("container.spotlight.back")));
		buttonList.add(save = new GuiButton(1, x + 5, y + 20, 166, 20, I18n.format("container.spotlight.save")));
		buttonList.add(load = new GuiButton(2, x + 5, y + 43, 166, 20, I18n.format("container.spotlight.load")));
		buttonList.add(delete = new GuiButton(3, x + 5, y + 66, 166, 20, I18n.format("container.spotlight.delete")));
		buttonList.add(new GuiButton(4, x + 5, y + 89, 166, 20, I18n.format("container.spotlight.reset")));
	...
	protected void actionPerformed(GuiButton guibutton)
	{
		...
		case 1:
		{
			mc.displayGuiScreen(new GuiSpotLightSaveConfig(invPlayer, tileSpotLight, world));
			break;
		}
		case 2:
		{
	...
	{
		save.enabled = tileSpotLight.getStackInSlot(0) != null && tileSpotLight.getStackInSlot(0).getItem() == TSMItems.configSaver;
		load.enabled = save.enabled;
		delete.enabled = save.enabled;
	...
	public void drawScreen(int mouseX, int mouseY, float partialRenderTick)
	{
		...
		if(helpButton.getIsActive())
		{
			...
			if(mouseX > x + 5 && mouseX < x + 171)
			{
				...
				{
					list = UtilSpotLight.formatedText(fontRendererObj, I18n.format("tutorial.spotlight.configs.save"), mouseX, width, reversed);
				}

				if(mouseY > y + 43 && mouseY < y + 63)
				{
					list = UtilSpotLight.formatedText(fontRendererObj, I18n.format("tutorial.spotlight.configs.load"), mouseX, width, reversed);
				}

				if(mouseY > y + 66 && mouseY < y + 86)
				{
					list = UtilSpotLight.formatedText(fontRendererObj, I18n.format("tutorial.spotlight.configs.delete"), mouseX, width, reversed);
				}

				if(mouseY > y + 89 && mouseY < y + 109)
				{
					list = UtilSpotLight.formatedText(fontRendererObj, I18n.format("tutorial.spotlight.configs.reset"), mouseX, width, reversed);
				}
			}
	...
	protected void drawGuiContainerBackgroundLayer(float partialRenderTick, int mouseX, int mouseYS)
	{
		...
		fontRendererObj.drawString(I18n.format("container.spotlight.desc", I18n.format("container.spotlight.configs")), x + 5, y + 8, 4210752);
	}
}